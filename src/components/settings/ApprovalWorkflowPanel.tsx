...
export const ApprovalWorkflowPanel: React.FC<ApprovalWorkflowPanelProps> = ({
  ...
  const getStepIcon = (index: number) => {
    switch (index) {
      ...
      default:
        return <Workflow className="w-5 h-5 text-slate-400" />;
    }
  };
  ...
  return (
    <div className="space-y-6" id="settings-panel-workflow">
      <div className="border-b border-white/5 pb-4">
        <h3 className="text-base font-semibold text-white">Approval Hierarchy & Delegation Workflow</h3>
        ...
        <div className="space-y-3 max-w-xl mx-auto py-2">
          {settings.steps.map((step, idx) => (
            <React.Fragment key={step.stepNumber}>
              <div className="p-4 rounded-2xl bg-[#0f1016] border border-white/5 flex items-center gap-4 relative shadow-md">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-white/5 text-slate-400 font-mono">
                      Step {step.stepNumber}
                    </span>
                    <h5 className="text-xs font-semibold text-white">{step.roleTitle}</h5>
                  </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-between gap-4">
            <div className="space-y-0.5">
              <div className="text-xs font-semibold text-white">Self-Approval Prohibition</div>
              <p className="text-[11px] text-slate-400">
            ...
            <button
              ...
              className={`w-11 h-6 rounded-full p-1 transition-colors relative cursor-pointer shrink-0 ${
                !settings.allowSelfApproval ? 'bg-indigo-600' : 'bg-white/10'
              }`}
