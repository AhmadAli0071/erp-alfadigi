  ...
  return (
    <div className="space-y-6" id="settings-panel-notifications">
      <div className="border-b border-slate-200/70 pb-4">
        <h3 className="text-base font-semibold text-slate-900">Event Notification Preferences</h3>
      ...
      <div className="space-y-3">
        {toggles.map((item) => {
          ...
          return (
            <div
              key={item.key}
              className="p-4 rounded-xl bg-slate-50 border border-slate-200/70 flex items-center justify-between gap-4"
            >
              ...
              <button
                ...
                className={`w-11 h-6 rounded-full p-1 transition-colors relative cursor-pointer shrink-0 ${
                  isEnabled ? 'bg-indigo-600' : 'bg-slate-200/50'
                }`}
