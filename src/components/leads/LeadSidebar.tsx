...
export const LeadSidebar: React.FC<LeadSidebarProps> = ({
  ...
  const isRouteActive = (route: string) => {
    if (route === '/lead/dashboard') {
      return currentRoute === '/lead/dashboard' || currentRoute === '/' || !currentRoute;
    }
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white/75 backdrop-blur-xl border-r border-slate-200/70 text-slate-600 select-none">
      <div className="h-16 flex items-center justify-between px-4 border-b border-slate-200/70 shrink-0">
        {!isCollapsed ? (
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => handleItemClick('/lead/dashboard')}>
            <BrandLogo size="sm" />
          </div>
        ) : (
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1.5 custom-scrollbar" aria-label="Lead Sidebar Navigation" id="lead-sidebar-nav">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {!isCollapsed && item.badge !== undefined && (
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${item.badgeColor || 'bg-slate-200/50 text-slate-600'}`}>
                  {item.badge}
                </span>
              )}
            </button>
      ...
      {!isCollapsed && (
        <div className="p-3 m-3 rounded-xl bg-slate-50 border border-slate-200/70 shrink-0 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-medium text-slate-600">{portalLabel}</span>
          </div>
  ...
  return (
    <>
      <aside className={`hidden lg:block shrink-0 transition-all duration-300 z-30 sticky top-0 h-screen ${isCollapsed ? 'w-20' : 'w-60'}`} id="lead-desktop-sidebar">
        {sidebarContent}
      </aside>
