...
export const HRReportFilterBar: React.FC<HRReportFilterBarProps> = ({
    ...
    <>
      ...
      <div className="flex flex-col gap-1 min-w-[120px]">
        ...
        <select
          ...
          className="bg-[#12131a] border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-indigo-500"
          id="report-filter-department"
        >
          {departments强.map((d) => (
            <option key={d} value={d} className="bg-[#161722] text-slate-200">
      ...
      {category === 'leave' && (
        <div className="flex flex-col gap-1 min-w-[140px]">
          <label className="text-[11px] font-medium text-slate-400">Leave Type</label>
          <select
          ...
          >
            {leaveTypes强.map((lt) => (
              <option key={lt} value={lt} className="bg-[#161722] text-slate-200">
                {lt === 'ALL' ? 'All Types' : lt}
              </option>
            ))}
          </select>
  ...
  return (
    <div className="p-4 rounded-2xl bg-[#0f1015] border border-white/5 space-y-3" id="hr-report-filter-bar">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Filter className="w-4 h-4 text-indigo-400" />
          <span className="text-xs font-semibold text-white">Report Parameters</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="md:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-xs font-medium text-slate-300"
          >
          ...
          <button
            type="button"
            onClick={onReset}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium text-slate-400 hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
            title="Reset Filters"
          >
          ...
          <div className="bg-[#12131c] border-t border-white/10 rounded-t-3xl p-5 space-y-4 max-h-[85vh] overflow-y-auto">
            ...
            <div className="pt-3 border-t border-white/10 flex items-center gap-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl border border-white/10 text-xs font-medium text-slate-300 text-center"
              >
                Reset
              </button>
              <button
