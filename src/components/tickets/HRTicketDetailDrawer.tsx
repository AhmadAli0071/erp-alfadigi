...
export const HRTicketDetailDrawer: React.FC<HRTicketDetailDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/80 backdrop-blur-sm animate-fadeIn">
      <div
        className="w-full md:w-[600px] lg:w-[680px] h-full bg-[#0d0e14] border-l border-white/10 flex flex-col shadow-2xl overflow-hidden"
        id="hr-ticket-detail-drawer"
      >
        {/* Top Header */}
        <div className="p-4 sm:p-5 border-b border-white/10 flex items-center justify-between gap-3 shrink-0 bg-white/[0.02]">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="px-2.5 py-1 rounded-lg bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 font-mono text-xs font-bold shrink-0">
              {ticket.ticketCode}
            </span>
            <div className="min-w-0">
              <h3 className="text-sm font-semibold text-white truncate">{ticket.subject}</h3>
              <p className="text-[11px] text-slate-400 truncate">{ticket.ticketType}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 space-y-1">
              ...
              <div className="text-xs font-semibold text-white">{ticket.status}</div>
            </div>
            <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 space-y-1">
              <span className="text-[10px] uppercase font-bold text-slate-500">Priority</span>
              <div className="text-xs font-semibold text-amber-400">{ticket.priority}</div>
            </div>
            <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 space-y-1">
              <span className="text-[10px] uppercase font-bold text-slate-500">Department</span>
              <div className="text-xs font-semibold text-slate-200">{ticket.department}</div>
            </div>
            <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 space-y-1">
              <span className="text-[10px] uppercase font-bold text-slate-500">Created</span>
              <div className="text-xs font-semibold text-slate-300">{ticket.createdDate}</div>
            </div>
          </div>

          {/* Employee & Assignee Information */}
          <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-indigo-600/20 border border-indigo-500/30 flex items-center justify-center text-indigo-400 shrink-0">
                <User className="w-5 h-5" />
              </div>
              <div>
                <div className="text-xs font-semibold text-white">{ticket.employeeName}</div>
            ...
            <div className="flex items-center gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-white/5">
              <div className="text-right">
                <div className="text-[10px] uppercase font-bold text-slate-500">Assigned To</div>
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-xs font-semibold text-slate-300 border border-white/10 transition-colors"
              >
                Assign
              </button>
            </div>
          ...
          {isAssigning && (
            <div className="p-4 rounded-xl bg-indigo-950/40 border border-indigo-500/30 space-y-3">
              <div className="text-xs font-semibold text-indigo-200">Assign Ticket</div>
              <input
                type="text"
                placeholder="Enter HR or Lead name..."
                value={assigneeName}
                onChange={(e) => setAssigneeName(e.target.value)}
                className="w-full bg-[#12131a] border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500"
              />
          ...
          <div className="space-y-2">
            ...
            <div className="p-4 rounded-xl bg-[#12131b] border border-white/5 text-xs text-slate-300 leading-relaxed whitespace-pre-wrap">
          ...
          <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="px-3 py-2 rounded-xl bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/30 text-xs font-semibold transition-colors"
              >
                Resolve Ticket
              </button>
              <button
                ...
                className="px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10 text-xs font-semibold transition-colors"
              >
                Close Ticket
              </button>
            </div>
            ...
            {activeTab === 'conversation' ? (
              <div className="space-y-3">
                ...
                {!ticket.messages || ticket.messages.length === 0 ? (
                  <div className="py-8 text-center text-xs text-slate-500 bg-white/[0.01] rounded-xl border border-white/5">
                    No messages yet
                  </div>
                ) : (
                ...
                <form onSubmit={handleSendMessage} className="flex gap-2 pt-2">
                  <input
                    ...
                    className="flex-1 bg-[#12131a] border border-white/10 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500"
                  />
                  <button
                    type="submit"
                  ...
                  <div className="space-y-3 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-px before:bg-white/10">
                    {ticket.activity.map((act) => (
                      <div key={act.id} className="relative pl-7 text-xs space-y-0.5">
                        <div className="absolute left-1.5 top-1 w-3 h-3 rounded-full bg-indigo-500/20 border border-indigo-400" />
                        ...
                        <div className="text-slate-500 text-[10px] font-mono">By: {act.actorName}</div>
                      </div>
                    ))}
                  </div>
                )}
