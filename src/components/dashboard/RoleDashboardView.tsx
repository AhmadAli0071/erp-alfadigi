...
export const RoleDashboardView: React.FC<RoleDashboardViewProps> = ({
  ...
  return (
    <div
      className="min-h-screen bg-[#0a0a0b] text-slate-200 flex flex-col justify-between"
      id="role-dashboard-container"
    >
      {/* Top Navigation Bar */}
      <header className="bg-[#0f1014] border-b border-white/5 px-6 py-4 sticky top-0 z-20">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex flex-col text-right">
              ...
              <span className="text-xs text-slate-400 font-medium">{user.jobTitle}</span>
            </div>

            <button
              onClick={onLogout}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-rose-300 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 transition-all focus:outline-none focus:ring-2 focus:ring-rose-500 cursor-pointer"
      ...
      <main className="max-w-6xl w-full mx-auto px-4 sm:px-6 py-10 flex-1">
        ...
        <div className="mb-8 p-4 sm:p-5 rounded-2xl bg-emerald-950/30 border border-emerald-500/30 text-emerald-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-lg">
          ...
          <button
            onClick={onLogout}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-300 hover:text-emerald-100 underline underline-offset-4 focus:outline-none cursor-pointer"
        ...
        <div className="bg-[#121318] rounded-2xl border border-white/10 p-6 sm:p-8 shadow-xl mb-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2.5 mb-1.5">
                  <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight" id="dashboard-role-heading">
                    {info.title}
                  </h1>
                  <span
                ...
                <p className="text-sm font-medium text-slate-400">{info.subtitle}</p>
              </div>
            </div>

            {/* Department / Status Pill */}
            {user.department && (
              <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white/[0.03] border border-white/10 text-slate-300 text-xs font-medium self-start md:self-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
              <span className="text-xs font-medium text-slate-500 block mb-1">Authenticated User</span>
              <span className="text-sm font-bold text-white">{user.name}</span>
            </div>

            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
              <span className="text-xs font-medium text-slate-500 block mb-1">Work Email</span>
              <span className="text-sm font-mono text-slate-300 truncate block flex items-center gap-1.5">
            ...
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
              <span className="text-xs font-medium text-slate-500 block mb-1">Assigned Role Key</span>
              <span className="text-sm font-mono font-bold text-indigo-400">{user.role}</span>
            </div>

            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
              <span className="text-xs font-medium text-slate-500 block mb-1">Designation</span>
              <span className="text-sm font-semibold text-slate-200">{user.jobTitle}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 bg-[#121318] rounded-2xl border border-white/10 p-6 shadow-xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {info.permissions.map((perm, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-2.5 p-3 rounded-xl bg-white/[0.02] border border-white/5 text-xs font-semibold text-slate-300"
                >
          ...
          <div className="bg-[#0f1014] border border-white/10 rounded-2xl p-6 text-white shadow-xl flex flex-col justify-between">
            <div>
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-indigo-500/20 text-indigo-300 text-[11px] font-semibold mb-3 border border-indigo-500/30">
                <BadgeAlert className="w-3.5 h-3.5" />
                <span>Phase 1 Scope: Login Module</span>
              </div>
              <h4 className="text-base font-bold text-white mb-2">Phase 1 Complete</h4>
      ...
      {/* Footer */}
      <footer className="bg-[#0f1014] border-t border-white/5 py-4 px-6 text-center text-xs text-slate-500">
